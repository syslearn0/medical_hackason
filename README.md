...
3. 対応表で違う点数が複数出た場合は、Qwenの推定を保持して要確認にします。単純に最低値・最大値にはしません。
   リスクだけは単文で決まらないため、`risk_rule()` が文の組み合わせで決めます（「注意点として〜高リスク」の列挙=4、服薬拒否が重なれば5、経過観察・注意点の列挙=3、介助が多い=3・安定の記載があれば2、介助あり=1、自立=0）。公開3名の傾向から作った暫定規則で、境界は未確定です。
   介助負担は、対応表の文がない日だけ `burden_rule()` で決めます（入浴か排泄に介助=3、入浴・排泄とも自立=0）。「入浴または排泄で介助が必要」は対応表で4です。
